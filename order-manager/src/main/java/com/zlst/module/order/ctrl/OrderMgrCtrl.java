...
import com.zlst.database.core.bean.vo.param.SearchWithPageParam;
...
public class OrderMgrCtrl {
    ...
    private void cancelProcInstanceInfo(String processInstId, String reason) {
        String result = orderMgrService.cancelProcInstance(processInstId, reason);
        //正确时返回空字符串，否则都视为异常
    ...
    public Object createOrder(@RequestBody OmsOrderRequest omsOrderRequest) {
        ...
        try {
            ...
            orderId = orderMgrService.saveProcessAndOrder(omsOrderRequest,omsOrder, getProcessDefId(omsOrderRequest.getBusiType()));
